    ...
    CompositionLocalProvider(LocalLayoutDirection provides if (hebrew) LayoutDirection.Rtl else LayoutDirection.Ltr) {
        Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.secondaryContainer)) {
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                if (updates.failed) Text(tr(label("לא הצלחנו לעדכן כרגע. אפשר לנסות שוב מאוחר יותר.",
                ...
                if (!downloading) FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(onClick = updates::update, enabled = !updates.pending) {
                        Text(tr(if (ready) label("התקן והפעל מחדש", "Install and restart") else label("עדכן", "Update")))
                    }
                    TextButton(onClick = updates::later, enabled = !updates.pending) { Text(tr(label("אחר כך", "Later"))) }
                }
            }
        }
    }
