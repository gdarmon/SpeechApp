...
export async function reportContent(store:Store,user:string,input:z.infer<typeof contentReportSchema>) {
  ...
  const id=randomUUID();
  await store.query('INSERT INTO fala.content_reports(id,user_id,session_id,target_key,category,note,content) VALUES($1::uuid,$2::uuid,$3::uuid,$4,$5,$6,$7::text::jsonb)',[id,user,input.session_id,key,input.category,input.note,JSON.stringify(content)]);
