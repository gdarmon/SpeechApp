...
allowed = [store / name for name, _ in images] + [store / 'preview.html', store / 'asset-manifest.json', store / 'app-access.txt', store / 'console-answers.md', store / 'README.md'] + list((store / 'en-US').glob('*.txt')) + list((store / 'screenshots').glob('*/capture.json'))
