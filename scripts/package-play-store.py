...
html += '<nav><a href="en-US/full-description.txt">Listing text</a><a href="app-access.txt">Reviewer instructions</a><a href="console-answers.md">Console setup answers</a><a href="closed-test-plan.md">Closed-test plan</a><a href="asset-manifest.json">Sizes and checksums</a></nav><section><img class="banner" src="feature-graphic-1024x500.jpg" alt="Fala: Speak a little. Grow a little. Portuguese conversation speech bubbles"></section>'
...
allowed = [store / name for name, _ in images] + [store / 'preview.html', store / 'asset-manifest.json', store / 'app-access.txt', store / 'console-answers.md', store / 'closed-test-plan.md', store / 'README.md'] + list((store / 'en-US').glob('*.txt')) + list((store / 'screenshots').glob('*/capture.json'))
