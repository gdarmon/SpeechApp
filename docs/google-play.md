...
Prepared application: **Fala**, package **com.fala.app**, version **0.12.2** (local version code 15; CI assigns a higher code), Android 8.0+, target SDK 36. Confirm package availability when creating the app; a first upload fixes the package identity. Do not change it after publishing.
